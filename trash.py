...
pVolume = subprocess.Popen("amixer set 'Master' 60% 2> /dev/null", shell=True)
...
print("Starting trash mode...")
subprocess.Popen("espeak \"Give me your trash to dispose\" -p 2 -g 5 2> /dev/null", shell=True)
time.sleep(1)
# set directions
aDir(True)
bDir(True)
...
while True:
	...
	if currdist > 1:
		...
		if input_state == False:
			print('Trash Mode ended Pressed')
			subprocess.Popen("espeak \"Trash eliminated. I'm coming back.\" -p 2 -g 5", shell=True)
			time.sleep(0.5)
			pwmA.ChangeDutyCycle(0)
			pwmB.ChangeDutyCycle(0)
			pwmC.ChangeDutyCycle(0)
